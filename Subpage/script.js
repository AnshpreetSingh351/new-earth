    ...
    projects: [
      {
        ...
        url: "https://www.cminfiniaimmersive.com", // 🔥 ADD THIS
    ...
    const customLayer = {
      ...
      onAdd(map, gl) {
        ...
        carLoader.load("car.glb", (gltf) => {
          ...
          // 🔥 AUTO SPAWN if project already selected
          if (currentProject) {
            spawnCars(currentProject);
          }
        });
      ...
      render(gl, matrix) {
        ...
        cars.forEach(car => {
          ...
          const start = carPaths[0];
          const end = carPaths[1];

          if (!start || !end) return;

          // 🔥 interpolate path
          const lng = start[0] + (end[0] - start[0]) * car.t;
          const lat = start[1] + (end[1] - start[1]) * car.t;
          ...
          // 🔥 FINAL POSITION (CORRECT SPACE)
          car.mesh.position.set(
            (merc.x - base.x) + pos[0] * scale,
            (merc.y - base.y) + pos[1] * scale,
            pos[2] * scale + 8   // 🔥 lifted above ground
          );

          // 🔥 SCALE (VISIBLE)
          car.mesh.scale.set(scale * 80, scale * 80, scale * 80);

          // 🔥 ROTATION (aligned)
          car.mesh.rotation.set(Math.PI / 2, 0, Math.PI / 2);
...
function createCarPath(project) {
  return [
    [project.coords[0] - 0.001, project.coords[1]],
    [project.coords[0] + 0.001, project.coords[1]]
  ];
}

function spawnCars(project) {

  if (!carModelTemplate || !scene) {
    console.log("Car model not ready yet ❌");
    return;
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // 🔥 VERY IMPORTANT (fix invisible materials)
    car.traverse(child => {
      if (child.isMesh) {
  ...
  carPaths = createCarPath(project);

  console.log("Cars spawned ✅");
}
